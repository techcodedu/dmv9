...
@section('title', 'Incoming Records')
                ...
                @else
                    ...
                    <tbody>
                        @foreach($incomingDocuments as $incomingDocument)
                            <tr>
                                <td>{{ $incomingDocument->document->document_id }}</td>
                                <td>{{ $incomingDocument->document->document_type }}</td>
                                <td>{{ $incomingDocument->document->status }}</td>
                                <td>{{ $incomingDocument->document->department->name }}</td>
                                <td>{{ $incomingDocument->forwardedBy->name }}</td>
                                <td>{{ $incomingDocument->fromOffice->name }}</td>
                                <td>{{ \Carbon\Carbon::parse($incomingDocument->date_forwarded)->format('F j, Y \a\t g:i a') }}</td>
                                <td>
                                   @foreach($incomingDocument->document->files as $file)
                                        <a href="#" onclick="openModalAndDisplayPDF('{{ asset('documents/' . $file->filename) }}', '{{ $incomingDocument->document->document_id }}')">Preview</a>
                                    @endforeach
                                </td>
                            </tr>
                        @endforeach
